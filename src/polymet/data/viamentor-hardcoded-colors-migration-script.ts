...
create_backup() {
  ...
  find ./polymet -type f \( -name "*.tsx" -o -name "*.jsx" \) -exec cp --parents {} "$BACKUP_DIR" \;
...
replace_in_files() {
  ...
  done < <(find ./polymet -type f \( -name "*.tsx" -o -name "*.jsx" \))
...
if [ ! -d "./polymet" ]; then
  print_error "Erreur: Répertoire ./polymet non trouvé"
...
echo "  • grep -r 'bg-green-500' ./polymet"
echo "  • grep -r 'text-blue-500' ./polymet"
echo "  • grep -r 'border-red-200' ./polymet"
echo "  • grep -r 'bg-orange-50' ./polymet"
echo "  • grep -r 'text-yellow-600' ./polymet"
echo ""

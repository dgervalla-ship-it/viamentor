/**
 * VIAMENTOR Secretary Tour
 *
 * Tour guidé 6 steps pour Secrétaire
 * Focus sur tâches, inscription, planning, messages, élèves
 *
 * @module components/viamentor-tour-secretary
 * @version 1.0.0
 */
...
import { getTourTranslations } from "@/polymet/data/viamentor-tours-i18n";
import {
  shouldShowTour,
  markTourCompleted,
  markTourSkipped,
  ...
  type TourStep,
  type TourAnalytics,
} from "@/polymet/data/viamentor-tours-types";
import { UserRole } from "@/polymet/data/viamentor-roles";

export interface SecretaryTourProps {
...
/**
 * Tour Secretary - 6 steps
 */
export function SecretaryTour({
  locale = "fr",
  onComplete,
  onSkip,
}: SecretaryTourProps) {
  ...
  const tourId = "secretary-tour";
  const t = getTourTranslations("secretary", locale);

  const steps: TourStep[] = [
    {
      target: '[data-tour-id="secretary-tasks"]',
      title: t.steps.tasks.title,
      content: t.steps.tasks.content,
      placement: "bottom",
      disableBeacon: true,
    },
    {
      target: '[data-tour-id="quick-registration"]',
      title: t.steps.registration.title,
      content: t.steps.registration.content,
      placement: "bottom",
      spotlightClicks: true,
    },
    {
      target: '[data-tour-id="secretary-planning"]',
      title: t.steps.planning.title,
      content: t.steps.planning.content,
      placement: "right",
    },
    {
      target: '[data-tour-id="secretary-messages"]',
      title: t.steps.messages.title,
      content: t.steps.messages.content,
      placement: "left",
    },
    {
      target: '[data-tour-id="secretary-students"]',
